        ...
        while (!exit)
        {
            ...
            try
            {
                switch (choice)
                {
                    ...
                    case 13:
                        if (!Manager.IsAddressBookSelected()) break;
                        await Manager.GetCurrentAddressBook().WriteToFileAsync();
                        break;

                    case 14:
                        if (!Manager.IsAddressBookSelected()) break;
                        await Manager.GetCurrentAddressBook().ReadFromFileAsync();
                        break;

                    case 15:
                        if (!Manager.IsAddressBookSelected()) break;
                        await Manager.GetCurrentAddressBook().WriteToCsvFileAsync();
                        break;

                    case 16:
                        if (!Manager.IsAddressBookSelected()) break;
                        await Manager.GetCurrentAddressBook().ReadFromCsvFileAsync();
                        break;

                    case 17:
                        if (!Manager.IsAddressBookSelected()) break;
                        await Manager.GetCurrentAddressBook().WriteToJsonFileAsync();
                        break;

                    case 18:
                        if (!Manager.IsAddressBookSelected()) break;
                        await Manager.GetCurrentAddressBook().ReadFromJsonFileAsync();
                        break;
                    case 19:
